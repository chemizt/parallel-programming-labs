    ...
    do
    {
        ...
        switch (versatileVar)
        {
            ...
            case 2:
            {
                ...
                {
                    uInt threadsQuantity = t;
                    vector<int> tmpDurVector;
                    vector<double> tmpAccelVector;

                    if (threadsQuantity == 0)
                    {
                        cout << "Количество процессов/потоков: " << threadsQuantity << " (линейный вариант)\n\n";
                    }
                    else
                    {
                    ...
                    for (uInt p = 0; p <= WORKSET_STEP_COUNT; p++)
                    {
                        ...
                        {
                            system(("mpiexec -n 6 worker " + std::to_string(worksetSize) + " " + std::to_string(threadsQuantity)).c_str());
            ...
            case 3:
            {
                ...
                else
                {
                    ...
                    << "а по вертикали - в порядке возрастания числа потоков (от 1 до 8 включительно). "
