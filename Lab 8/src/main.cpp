...
int main()
{
    ...
    do 
    {
        ...
        << "3. Тестовый прогон (критические секции)\n"
        << "4. Тестовый прогон (замки)\n"
        ...
        switch (versatileVar)
        {
            ...
            case 3:
            {
                cout << "Выполняется тестовый прогон программы с использованием критических секций\n"
                ...
                logOutput("Контрольный прогон завершён!\n");
                break;
            }
            case 4:
            {
                cout << "Выполняется тестовый прогон программы с использованием замков\n"
                << "Количество итераций для каждой сущности: " << TEST_ITERATIONS<< endl;
                omp_init_lock(&lock);
                #pragma omp parallel sections num_threads(2)
                {
